...
def get_mongodb_connection():
    ...
    try:
        ...
        logger.log("✓ Connected to MongoDB Atlas", "SUCCESS")
        return client
    except Exception as e:
        logger.log(f"✗ MongoDB connection failed: {e}", "ERROR")
        raise
...
def fetch_proposals_from_synthesis_architect(db_client) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.log(f"✓ Retrieved {len(proposals)} proposals from Synthesis Architect", "SUCCESS")
...
def check_environment():
    ...
    mongodb_uri = os.getenv("MONGODB_URI")
    if mongodb_uri:
        logger.log(f"✓ MONGODB_URI set", "SUCCESS")
    else:
        logger.log("✗ MONGODB_URI not set", "ERROR")
        return False
    ...
    if gemini_key and gemini_key != "your_gemini_api_key_here":
        logger.log(f"✓ GEMINI_API_KEY set (Synthesis Architect enabled)", "SUCCESS")
    else:
        logger.log("⚠ GEMINI_API_KEY not set (Synthesis Architect disabled)", "WARNING")
...
def main():
    ...
    logger.log("  1. Synthesis Architect (Gemini) → generates project proposals", "INFO")
    logger.log("  2. LangGraph Manager (DeepSeek) → coordinates ranking workflow", "INFO")
    logger.log("  3. MongoDB Atlas → shared memory across agents", "INFO")
    ...
    if not projects:
        logger.log("⚠ No proposals found. Using sample projects...", "WARNING")
